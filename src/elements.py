...
class CanvasButton(Text, tk.Canvas):
    """Button emulation based on Canvas() widget. Can have text
    and/or preconfigured image."""
    ...
    def __init__(self, master=None, image=None, text=None, variable=None,
        ...
        standard_options = {}
        for item in (
        'width', 'height', 'relief', 'bg', 'bd', 'state', 'takefocus'):
            if eval(
                    item) is not None:  # Check because value of item can be 0.
        ...
        self.bind("<Configure>",
                  self._place)  # Need to be before call of config_button()!
        # Configure widget with specific options:
        self.config_button(image=image, text=text, variable=variable,
                           textwidth=textwidth, state=state,
                           textheight=textheight, fontsize=fontsize,
                           opacity=opacity, bg=bg, command=command)
        ...
        if not height:
            self.config(height=items_height + (
                (items_height / 5) if image else (
                            items_height / 2)) + bdsize * 2)
    ...
    def config_button(self, **kwargs):
        ...
        if 'image' in kwargs and kwargs['image']:
            self.add_image(kwargs['image'],
                           opacity='right' if 'opacity' not in kwargs else
                           kwargs['opacity'])
        ...
        if text:
            self.add_text(text, **{key: kwargs[key] for key in (
            'fontsize', 'textwidth', 'textheight', 'bg', 'opacity')
                                   if key in kwargs})
    ...
    def add_image(self, image, opacity='right'):
        ...
        if self.bbox('image'):
            coords = self.coords(
                'image')  # New image will appear in the same position as previous.
            self.delete('image')
        self.picture = tk.PhotoImage(
            file=image)  # 'self' need to override garbage collection action.
        self.create_image(coords[0], coords[1], image=self.picture,
                          anchor='nw', tag='image')

    def add_text(self, textorvariable, fontsize=None, bg=None, opacity="right",
                 textwidth=None, textheight=None):
        ...
        if isinstance(textorvariable, tk.Variable):
            ...
            self.textlabel = tk.Label(self, text=textorvariable, bd=0,
                                      bg=self.bg, font=font, justify='center',
                                      state=self.cget('state'),
                                      width=textwidth, height=textheight)
        if self.bbox('image'):
            ...
            y_multiplier = ((self.bbox('image')[3] - self.bbox('image')[
                1]) - self.textlabel.winfo_reqheight()) / 2
        else:
            x_multiplier = x_divider = y_multiplier = 0
        self.create_window(coords[0] if recreate else x_multiplier + x_divider,
                           coords[1] if recreate else y_multiplier,
    ...
    def release_button(self, event):
        ...
        if self.cget('state') == 'normal' and self.pressed:
            ...
            self.move('all', -1, -1)
            if callable(self.command) and event.x_root in range(
                    self.winfo_rootx(), self.winfo_rootx() +
                                        self.winfo_width()) and event.y_root \
                    in range(self.winfo_rooty(), self.winfo_rooty() +
                                    self.winfo_height()):
                self.command()
        self.pressed = False
...
class ScrolledCanvas(tk.Frame):
    ...
    def __init__(self, parent=None, orientation="vertical", bd=2, **options):
        ...
        scroller.config(command=(
            self.canvbox.xview if orientation == "horizontal"
            else self.canvbox.yview))
        if orientation == "horizontal":
            self.canvbox.config(xscrollcommand=scroller.set)
        else:
            self.canvbox.config(yscrollcommand=scroller.set)
        scroller.pack(fill='x' if orientation == 'horizontal' else 'y',
                      expand=1,
        ...
        self.canvbox.create_window((0, 0), window=self.content_frame,
                                   anchor='nw')
